...
#+BEGIN_SRC python :results output :file paquetes.el
  ...
  elisp_template = """;; Install extensions if they're missing
  ...
  if os.path.isdir(os.path.expanduser("~/.emacs.d/elpa/")):
      ...
      packagestoinstall.remove("archive")  # This is not a package!

      # Sort things, please!
      packagestoinstall.sort()
      print(elisp_template.format(firstpackage=packagestoinstall[0],
                                  packages=textwrap.indent(
                                      "\n".join(packagestoinstall[1:]), "     ")))
#+END_SRC
